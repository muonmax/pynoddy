            ...
            for i in range(0, self.nx):
                for j in range(0, self.ny - 1):
                    ...
                    if self.block[i][j][0] != self.block[i][j + 1][0]:  # this is a contact
                        code = "%d_%d" % (min(self.block[i][j][0], self.block[i][j + 1][0]),
                                          max(self.block[i][j][0], self.block[i][j + 1][0]))
                        if code not in x:
                            x[code] = []
                            y[code] = []
                            z[code] = []
            ...
            for i in range(0, self.nx):
                for j in range(0, self.nz - 1):
                    ...
                    if self.block[i][0][j] != self.block[i][0][j + 1]:  # this is a contact
                        code = "%d_%d" % (min(self.block[i][0][j], self.block[i][0][j + 1]),
                                          max(self.block[i][0][j], self.block[i][0][j + 1]))
                        if code not in x:
                            x[code] = []
                            y[code] = []
                            z[code] = []
            ...
            for i in range(0, self.ny):
                for j in range(0, self.nz - 1):
                    ...
                    if self.block[0][i][j] != self.block[0][i][j + 1]:  # this is a contact
                        code = "%d_%d" % (min(self.block[0][i][j], self.block[0][i][j + 1]),
                                          max(self.block[0][i][j], self.block[0][i][j + 1]))
                        if code not in x:
                            x[code] = []
                            y[code] = []
                            z[code] = []
...
def CalculatePlotStructure(modelfile, plot, noddy_path, includeGravityCalc=0,
    ...
    start = time.time()
    nSurfaces, points, CatCodes = convertSurfaces2VTK(points, cell_data, faceCounter, outputOption, fileprefix,
                                                      xy_origin=xy_origin)
    end = time.time()
    ...
    vol = vtkP.Volume(N1.block, c='jet', spacing=[N1.delx, N1.dely, N1.delz],
                      origin=[xy_origin[0] + N1.xmin, xy_origin[1] + N1.ymin, xy_origin[2] + N1.zmin])
...
class NoddyTopology(object):
    ...
    def loadNetwork(self):
        ...
        try:
            ...
            print(
                "Warning: NetworkX module could not be loaded. Please install NetworkX from https://networkx.github.io/ to perform topological analyses in PyNoddy")
        ...
        for l in lines:  # load edges
            if '_' in l:  # this line contains topology stuff (aka ignore empty lines)
                ...
                eCode = 0
                eAge = self.lithology_properties[int(lithoCode1)][
                    'age']  # for original stratigraphy. Default is the age of the first node
                ...
                self.graph.add_node(data[0], lithology=lithoCode1,
                                    name=self.lithology_properties[int(lithoCode1)]['name'],
                                    age=self.lithology_properties[int(lithoCode1)]['age'])
                self.graph.add_node(data[1], lithology=lithoCode2,
                                    name=self.lithology_properties[int(lithoCode2)]['name'],
                                    age=self.lithology_properties[int(lithoCode2)]['age'])

                if (self.load_attributes):
                    self.graph.node[data[0]]['colour'] = self.lithology_properties[int(lithoCode1)]['colour']
                    self.graph.node[data[0]]['centroid'] = self.node_properties["%d_%s" % (int(lithoCode1), topoCode1)][
                        'centroid']
                    self.graph.node[data[0]]['volume'] = self.node_properties["%d_%s" % (int(lithoCode1), topoCode1)][
                        'volume']

                    self.graph.node[data[1]]['colour'] = self.lithology_properties[int(lithoCode2)]['colour']
                    self.graph.node[data[1]]['centroid'] = self.node_properties["%d_%s" % (int(lithoCode2), topoCode2)][
                        'centroid']
                    self.graph.node[data[1]]['volume'] = self.node_properties["%d_%s" % (int(lithoCode2), topoCode2)][
                        'volume']

                # add edge
                self.graph.add_edge(data[0], data[1], name=name, edgeCode=eCode, edgeType=eType, colour=eColour,
                                    area=count, weight=1, age=eAge)
    ...
    def collapse_stratigraphy(self):
        ...
        **Returns**
        ...
        if not 'overall' in self.type:
            print(
                "Error: structural and lithological collapsed topologies can only be calculated from the overall topology")
            return
        ...
        for e in self.graph.edges(data=True):
            ...
            for i, n in enumerate([u, v]):
                if not topo.graph.has_node(n):
                    ...
                    topo.graph.node[n]['volume'] = topo.graph.node[u]['volume'] + self.graph.node[e[i]][
        ...
        **Returns**
        ...
        for e in self.graph.edges(data=True):
            ...
            if not topo.graph.has_node(u):  # new node, add
                ...
                topo.graph.node[u]['volume'] = topo.graph.node[u]['volume'] + self.graph.node[e[0]][
                    'volume']  # increment volume
            ...
            if not topo.graph.has_node(v):  # new node, add
                ...
                topo.graph.node[v]['volume'] = topo.graph.node[v]['volume'] + self.graph.node[e[1]][
                    'volume']  # increment volume
            ...
            for n in G.nodes():
                ...
                if not S.has_node(n):
                    ...
                    if 'centroid' in S.node[n]:
                        S.node[n]['centroid_list'] = (
                        [G.node[n]['centroid'][0]], [G.node[n]['centroid'][1]], [G.node[n]['centroid'][2]])
                        S.node[n]['centroid'] = (w_inc * S.node[n]['centroid'][0], w_inc * S.node[n]['centroid'][1],
                                                 w_inc * S.node[n]['centroid'][2])
            ...
            for e in G.edges(data=True):
                ...
                if not S.has_edge(e[0], e[1]):  # add new edge
                    ...
                    try:
                        ...
                        print(("Type error combining edge %s, %s. List was observed rather than float - %s" % (
                        e[0], e[1], str(s_e['area']))))
    ...
    def draw_mayavi_graph(G, **kwds):
        ...
        **Optional Keywords**:
        ...
        try:
            ...
            print(
                "Error loading mayavi package: mayavi is not installed or is not on the python path. To install with pip, use 'pip install mayavi' (or 'conda install mayavi'")
            return
        ...
        for k in list(edge_groups.keys()):
            ...
            pts2 = mlab.points3d(e[0][0], e[0][1], e[0][2], e[3], scale_factor=edge_thickness, scale_mode='none',
                                 resolution=5)
        ...
        **Optional Keywords**:
        ...
        for n in G2.nodes():
            ...
            if 'centroid' not in G2.node[n]:
                print(
                    "Error: node centroids are not defined. Please ensure this topology object has not been collapsed")
                return
